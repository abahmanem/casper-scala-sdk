...
object TestSecp256k1 extends App {
  ...
  //keys
  val secp256k1Keys = com.casper.sdk.crypto.KeyPair.loadFromPem(getClass.getResource("/crypto/secp256k1/put_deploy_secret_key.pem").getPath)

  val ed25519Keys = com.casper.sdk.crypto.KeyPair.loadFromPem(getClass.getResource("/crypto/secp256k1/put_deploy_secret_key.pem").getPath)
  ...
  val header = new DeployHeader(
    ...
    TimeUtil.timeStampString(System.currentTimeMillis()).getOrElse(""), "30m",
  ...
  val session = new Transfer(Seq(arg1, arg01, arg02))
